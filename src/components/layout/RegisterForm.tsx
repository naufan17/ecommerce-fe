...
const RegisterForm: React.FC = () => {
  ...
  const { isRegistered, loading, error } = useSelector((state: any) => state.auth);
  const navigate = useNavigate();

  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    dispatch(registerUser({ name, email, gender, password }));
  };
      ...
      <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            <button
              type="submit"
              className="bg-gray-900 text-white rounded-md w-full py-2 text-base font-medium hover:bg-gray-700"
            >
              {loading ? (
